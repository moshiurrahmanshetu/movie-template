...
function initSearchPage() {
  ...
  const noResultsHeading = document.getElementById('noResultsHeading');
  ...
  // Initialize query from URL search parameters e.g. /search?q=avatar
  ...
  const initialQuality = urlParams.get('quality') || 'all';

  if (searchInput && initialQ) searchInput.value = initialQ;
  if (genreSelect && initialGenre !== 'all') genreSelect.value = initialGenre.toLowerCase();
  if (languageSelect && initialLang !== 'all') languageSelect.value = initialLang.toLowerCase();
  if (yearSelect && initialYear !== 'all') yearSelect.value = initialYear;
  if (qualitySelect && initialQuality !== 'all') qualitySelect.value = initialQuality;
  ...
  function filterAndDisplay() {
    ...
    const lang = languageSelect ? languageSelect.value.toLowerCase() : 'all';
    ...
    if (rawQuery) newParams.set('q', rawQuery);
    if (genre !== 'all') newParams.set('genre', genre);
    if (lang !== 'all') newParams.set('lang', lang);
    if (yearVal !== 'all') newParams.set('year', yearVal);
    if (quality !== 'all') newParams.set('quality', quality);
    ...
    let filtered = MOVIES_DATABASE.filter(m => {
      ...
      if (lang !== 'all') {
        const mLanguage = (m.language || '').toLowerCase();
        if (lang === 'english') {
          matchesLang = mLanguage.includes('english');
        } else if (lang === 'dual') {
          matchesLang = mLanguage.includes('dual') || mLanguage.includes('+');
        } else {
          matchesLang = mLanguage.includes(lang);
        }
      }
      ...
      if (yearVal !== 'all') {
        if (yearVal === 'older') {
          matchesYear = Number(m.year) <= 2020;
        } else {
          matchesYear = String(m.year) === yearVal;
        }
      }
      ...
      return matchesQuery && matchesGenre && matchesLang && matchesYear && matchesQuality;
    });
    ...
    if (statusText) {
      if (rawQuery) {
        statusText.innerHTML = `Showing <span class="text-white fw-bold">${filtered.length}</span> movies for <span class="cs-badge-highlight">&ldquo;${escapeHTML(rawQuery)}&rdquo;</span>`;
      } else {
        statusText.innerHTML = `Showing <span class="text-white fw-bold">${filtered.length}</span> movies matching your criteria`;
      }
    }
    ...
    if (filtered.length === 0) {
      ...
      if (emptyState) {
        ...
        if (noResultsHeading) {
          noResultsHeading.textContent = rawQuery ? `No movies found for “${rawQuery}”` : 'No Movies Found';
        }
        if (noResultsMessage) {
          noResultsMessage.textContent = rawQuery 
            ? `We couldn't find any movie matching “${rawQuery}”. Try adjusting your keywords or clearing filter parameters.`
            : 'We couldn\'t find any movies matching your current filter combination.';
        }
      }
    } else {
  ...
  if (languageSelect) languageSelect.addEventListener('change', filterAndDisplay);
  if (yearSelect) yearSelect.addEventListener('change', filterAndDisplay);
...
/**
 * Trust Modals Controller (Privacy, Terms, Disclaimer, Contact & DMCA)
 * Lightweight, dynamically loaded Bootstrap modals for trust navigation
 */
function initTrustModals() {
  if (document.getElementById('csTrustModalsContainer')) return;

  const container = document.createElement('div');
  container.id = 'csTrustModalsContainer';
  container.innerHTML = `
    <!-- Privacy Policy Modal -->
    <div class="modal fade" id="csPrivacyModal" tabindex="-1" aria-labelledby="csPrivacyTitle" aria-hidden="true">
      <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content cs-modal-content">
          <div class="modal-header cs-modal-header">
            <h5 class="modal-title text-white" id="csPrivacyTitle"><i class="bi bi-shield-lock text-accent me-2"></i> Privacy Policy</h5>
    ...
    <div class="modal fade" id="csTermsModal" tabindex="-1" aria-labelledby="csTermsTitle" aria-hidden="true">
      <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content cs-modal-content">
          <div class="modal-header cs-modal-header">
            <h5 class="modal-title text-white" id="csTermsTitle"><i class="bi bi-file-earmark-text text-accent me-2"></i> Terms of Service</h5>
            <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
          <div class="modal-body text-secondary small">
    ...
    <!-- Disclaimer Modal -->
    <div class="modal fade" id="csDisclaimerModal" tabindex="-1" aria-labelledby="csDisclaimerTitle" aria-hidden="true">
      <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content cs-modal-content">
          <div class="modal-header cs-modal-header">
            <h5 class="modal-title text-white" id="csDisclaimerTitle"><i class="bi bi-exclamation-triangle text-accent me-2"></i> Content & Copyright Disclaimer</h5>
            <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
          <div class="modal-body text-secondary small">
    ...
    <!-- Contact / DMCA Modal -->
    <div class="modal fade" id="csContactModal" tabindex="-1" aria-labelledby="csContactTitle" aria-hidden="true">
      <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content cs-modal-content">
          <div class="modal-header cs-modal-header">
            <h5 class="modal-title text-white" id="csContactTitle"><i class="bi bi-envelope-at text-accent me-2"></i> Contact & DMCA Compliance</h5>
            <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
          <div class="modal-body text-secondary small">
            <p class="text-white fw-bold mb-1">DMCA Takedown & Inquiries</p>
